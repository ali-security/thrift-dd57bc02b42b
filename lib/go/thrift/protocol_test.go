...
func TestReadContainerSizeOverflow(t *testing.T) {
	...
	// ParseElemListBegin backs both ReadListBegin and ReadSetBegin and reads the
	// count as a 64-bit value too, so it needs the same coverage as json/map.
	// OutputElemListBegin takes a platform int (too narrow on 32-bit builds), so
	// place the count on the wire with the lower-level writers, mirroring it.
	for _, elem := range []struct {
		name string
		read func(TProtocol) (TType, int, error)
	}{
		{"json/list", func(p TProtocol) (TType, int, error) { return p.ReadListBegin(ctx) }},
		{"json/set", func(p TProtocol) (TType, int, error) { return p.ReadSetBegin(ctx) }},
	} {
		t.Run(elem.name, func(t *testing.T) {
			// 0x4000000000000000 * 2 (min size of a STRING element) stays outside
			// int32 range when kept in 64 bits.
			const jsonOverflowSize int64 = 0x4000000000000000
			trans := NewTMemoryBuffer()
			w := NewTJSONProtocol(trans)
			if err := w.OutputListBegin(); err != nil {
			...
			es, _ := w.TypeIdToString(STRING)
			if err := w.WriteString(ctx, es); err != nil {
				t.Fatal(err)
			}
			if err := w.WriteI64(ctx, jsonOverflowSize); err != nil {
				t.Fatal(err)
			...
			r := NewTJSONProtocol(trans)
			r.SetTConfiguration(&TConfiguration{})
			if _, _, err := elem.read(r); err == nil {
				t.Error("expected error reading oversized list header, got nil")
			}
		})
	}
